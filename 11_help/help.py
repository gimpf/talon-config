...
def gui_context_help(gui: imgui.GUI):
    ...
    if selected_context is None:
        ...
        if not show_enabled_contexts_only:
            gui.text(
                "Help: All Contexts ({}/{})".format(current_context_page, total_page_count))
        else:
            gui.text(
                "Help: Active Contexts Only ({}/{})".format(current_context_page, total_page_count))
        ...
        for key in sorted_context_map_keys:
            ...
            if current_context_page == target_page:
                short_name = key.replace('(Context', '').replace(
                    ')', '').split('.')[-1].replace('_', " ")
                active = not show_enabled_contexts_only and key in cached_active_contexts_list
                button_name = f'{str(current_item_index)} | {short_name}{" [ACTIVE]" if active else ""}: {key}'
    ...
    # if there's a selected context, draw the commands for it
        ...
        if selected_context in cached_active_contexts_list:
            gui.text("{} ({}/{}) [ACTIVE]".format(selected_context,
                                                  selected_context_page, total_page_count))
        else:
            gui.text("{} ({}/{}) [INACTIVE]".format(selected_context,
                                                    selected_context_page, total_page_count))
        ...
        for key, val in context_mapping[selected_context].items():
            target_page = int(
                math.ceil(current_item_index / max_commands_per_page))
...
def refresh_context_mapping(enabled_only=False):
    ...
    for context in registry.contexts.values():
        ...
        short_name = context_name.replace('(Context', '').replace(
            ')', '').split('.')[-1].replace('_', " ")
